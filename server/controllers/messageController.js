const { MovieList, MovieCounts, MovieRatings } = require('../models/MovieModel');
const controller = {};

controller.postMovieCount = (req, res, next) => {
    const { data } = req.body;

    MovieCounts.updateOne({_id: data.tmdbId}, { $setOnInsert: {movie: data}, $inc: {count: 1}}, { upsert: true })
        .then ((result) => {
...
controller.postRating = (req, res, next) => {
    ...
    MovieRatings.updateOne({_id: movie.tmdbId}, { $setOnInsert: {movie: movie}, $inc: {totalRating: rating, count: 1}}, { upsert: true })
        .then (result => {
            res.locals.result = result;
            return next();
        })
...
controller.updateAvgRating = (req, res, next) => {
    ...
    MovieRatings.updateOne({_id: movie.tmdbId}, { avgRating: avgRating}, { upsert: true })
        .then (result => {
            res.locals.result = result;
            return next();
        })
...
controller.mostAdded = (req, res, next) => {
    MovieCounts.find({}, (err, result) => {
       ...
       return next()
    }).sort({count:-1}).limit(15);  //negative to sort from descending order
}
